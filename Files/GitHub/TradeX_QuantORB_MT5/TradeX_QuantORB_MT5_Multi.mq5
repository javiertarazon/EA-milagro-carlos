//+------------------------------------------------------------------+
//|                                   TradeX_QuantORB_MT5_Multi.mq5  |
//|              Opening Range Breakout (ORB) - Prop Firm EA         |
//|              Versión MULTI-OPERACIÓN: re-entrada tras cierre     |
//+------------------------------------------------------------------+
...
input int      InpMaxTradesDay  = 3;       // Máximo operaciones por día (re-entrada activa)
input int      InpReentryCooldownSec = 300; // Espera mínima entre operaciones (seg) | anti-spam
...
// Contadores de motivo de cierre (análisis de backtest)
int            g_c_tp = 0;
int            g_c_be = 0;
int            g_c_sl = 0;
int            g_c_oth = 0;
...
int OnInit()
{
   ...
   UpdateEMAObjects();
   ChartRedraw(0);
   Print("=== QuantORB MULTI EA Iniciado en ", _Symbol, " | Magic ", InpMagicNumber, " ===");
...
void OnDeinit(const int reason)
{
   ...
   DeleteORBObjects();
   DeleteEMAObjects();
   Print("=== QuantORB MULTI EA Detenido (razón ", reason, ") ===");
   if(MQLInfoInteger(MQL_TESTER))
   {
      // Análisis de cierres: OnTradeTransaction no se dispara en el tester,
      // así que se escanea el historial de la prueba al terminar.
      g_c_tp = 0;
      g_c_be = 0;
      g_c_sl = 0;
      g_c_oth = 0;
      if(HistorySelect(0, TimeCurrent()))
      {
         ...
         for(int i = 0; i < n; i++)
         {
            ...
            if(HistoryDealGetString(t, DEAL_SYMBOL) != _Symbol)
               continue;
            if(HistoryDealGetInteger(t, DEAL_MAGIC) != (long)InpMagicNumber)
               continue;
            ...
            string cm = HistoryDealGetString(t, DEAL_COMMENT);
            bool atTp = (tp > 0.0 && g_point > 0.0 && MathAbs(px - tp) <= 3.0 * g_point);
            bool atSl = (sl > 0.0 && g_point > 0.0 && MathAbs(px - sl) <= 10.0 * g_point);
            // Orden: etiquetas explícitas → TP → cierre en Stop → beneficio/pérdida
            if(StringFind(cm, "[tp") >= 0)
               g_c_tp++;
            else if(StringFind(cm, "[sl") >= 0)
            {
               if(p >= 0.0)
                  g_c_be++;   // SL movido (BE/Trailing) cerrado en beneficio
               else
                  g_c_sl++;   // SL original: cierre en pérdida
            }
            else if(atTp)
               g_c_tp++;      // el tester no etiqueta [tp]: precio de cierre ≈ TP
            else if(atSl)
            {
               // Cierre en nivel de Stop: pérdida → SL original; beneficio → BE/Trailing
               if(p < 0.0)
                  g_c_sl++;
               else
                  g_c_be++;
   ...
   PrintFormat("CIERRES_TOTAL|TP=%d|BE_TRAIL=%d|SL_PERPIDA=%d|OTROS=%d",
               g_c_tp, g_c_be, g_c_sl, g_c_oth);
}

//+------------------------------------------------------------------+
//| Clasifica el motivo de cierre de cada operación (solo análisis)  |
//+------------------------------------------------------------------+
void OnTradeTransaction(const MqlTradeTransaction &trans, const MqlTradeRequest &request, const MqlTradeResult &result)
{
   if(trans.type != TRADE_TRANSACTION_DEAL_ADD && trans.type != TRADE_TRANSACTION_DEAL_UPDATE)
      return;
   ulong deal = HistoryDealGetTicket(trans.deal);
   if(deal == 0)
      return;
   ...
   if(HistoryDealGetInteger(deal, DEAL_ENTRY) != DEAL_ENTRY_OUT)
      return; // solo interesan los cierres

   string cm = HistoryDealGetString(deal, DEAL_COMMENT);
   ...
   {
      if(profit >= 0.0)
         g_c_be++;   // SL movido por BE/Trailing y cerrado en beneficio
      else
         g_c_sl++;   // SL original: cierre en pérdida
   }
   else
      g_c_oth++;     // cierres manuales / EOD / otros
}
...
//+------------------------------------------------------------------+
//| Reconstruye el High/Low del ORB desde las velas M1 del tramo     |
//| (fallback si no se recibieron ticks en vivo durante la ventana)  |
//+------------------------------------------------------------------+
void RebuildORBRangeFromM1()
{
   if(g_orb_start <= 0 || g_orb_end <= 0)
      return;

   int startBar = iBarShift(_Symbol, PERIOD_M1, g_orb_start, false);
   if(startBar < 0)
      return;
   ...
   for(int i = startBar; i >= 0; i--)
   {
      ...
      if(t < g_orb_start)
         continue; // barra que arranca antes de la ventana: se ignora
      if(t >= g_orb_end)
         break;
   ...
   if(h > 0 && l > 0)
   {
      g_orb_high = h;
      g_orb_low  = l;
      PrintFormat("ORB reconstruido desde M1: High=%.5f Low=%.5f", h, l);
   }
   else
      Print("ORB sin datos: no se pudo reconstruir el rango desde M1.");
}
...
//+------------------------------------------------------------------+
//| ¿Hay alguna posición abierta de este EA (símbolo + magic) en la  |
//| dirección indicada?                                              |
//+------------------------------------------------------------------+
bool HasOpenDirection(ENUM_POSITION_TYPE dir)
{
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ulong ticket = PositionGetTicket(i);
...
//| MULTI: direcciones independientes + re-entrada tras cierre       |
//+------------------------------------------------------------------+
void CheckBreakout()
{
   if(g_orb_high <= 0 || g_orb_low <= 0)
   ...
   // Cooldown global entre operaciones (evita re-entradas en el mismo instante)
   if(g_last_entry_time > 0 && TimeCurrent() - g_last_entry_time < InpReentryCooldownSec)
      return;

   double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
   bool hasLong  = HasOpenDirection(POSITION_TYPE_BUY);
   bool hasShort = HasOpenDirection(POSITION_TYPE_SELL);

   // --- Entrada LONG: precio rompe el máximo y no hay long abierta ---
   if(!hasLong && bid > g_orb_high)
   {
      if(ATRFilterPassed() && RVOLFilterPassed())
         OpenTrade(ORDER_TYPE_BUY, bid, ask);
   }
   // --- Entrada SHORT: precio rompe el mínimo y no hay short abierta ---
   if(!hasShort && ask < g_orb_low)
   {
      if(ATRFilterPassed() && RVOLFilterPassed())
