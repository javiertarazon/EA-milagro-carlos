...
input int      InpCloseHour    = 16;       // Hora cierre EOD (broker)
input int      InpCloseMin     = 0;        // Minuto cierre EOD
...
input double   InpSLBufferPips  = 2.0;     // Buffer extra para SL (en puntos)
...
input double   InpBEActivatePips  = 8;       // Tope activación BE (puntos); máx. 50% del camino al TP
input double   InpBEPlusPips      = 2;       // Buffer del BE sobre la entrada (puntos)
input bool     InpUseTrailing     = true;    // ¿Usar Trailing Stop?
input double   InpTrailStartPips  = 12;      // Tope inicio trailing (puntos); máx. 75% del camino al TP
input double   InpTrailDistPips   = 6;       // Distancia del SL al precio al trailear (puntos)
input double   InpTrailStepPips   = 3;       // Paso mínimo para mover el SL (puntos)
...
double GetBETriggerPips(double entry, double tp)
{
   ...
   return(MathMin(InpBEActivatePips, autoPips));
}
...
double GetTrailTriggerPips(double entry, double tp)
{
   ...
   return(MathMin(InpTrailStartPips, autoPips));
}
...
void ManageBreakEvenAndTrailing()
{
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(type == POSITION_TYPE_BUY)
      {
         ...
         {
            double beSL = entry + InpBEPlusPips * g_point;
            if(curSL < beSL)
            {
               newSL = beSL;
               move  = true;
         ...
         {
            double trailSL = bid - InpTrailDistPips * g_point;
            if(trailSL > curSL + InpTrailStepPips * g_point)
            {
               newSL = trailSL;
               move  = true;
            }
      ...
      else if(type == POSITION_TYPE_SELL)
      {
         ...
         if(InpUseTrailing && profitPips >= GetTrailTriggerPips(entry, curTP))
         {
            double trailSL = ask + InpTrailDistPips * g_point;
            if(trailSL < curSL - InpTrailStepPips * g_point)
            {
               newSL = trailSL;
               move  = true;
...
bool RVOLFilterPassed()
{
   ...
   long vols[];
   if(CopyTickVolume(_Symbol, PERIOD_CURRENT, 1, InpRVOLPeriod, vols) < InpRVOLPeriod)
      return(true);
...
void OpenTrade(ENUM_ORDER_TYPE type, double bid, double ask)
{
   ...
   double slBase = (type == ORDER_TYPE_BUY) ? (g_orb_high - InpSLBufferPips * g_point)
                                            : (g_orb_low  + InpSLBufferPips * g_point);
...
double CalcLot(double entry, double sl)
{
   ...
   if(marginPerLot > 0 && freeMargin > 0)
   {
      ...
      if(maxLotByMargin > 0 && lot > maxLotByMargin)
         lot = maxLotByMargin;
      if(lot < lotMin)
         return(0);
   }
